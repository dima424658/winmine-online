...
{
	fTimer = fFalse;
	BOOL fAdjust = (Preferences.Width != xBoxMac || Preferences.Height != yBoxMac)
		? (fResize | fDisplay) : fDisplay;
...
void TrackMouseRec(int block, int xOld, int yOld, int xNew, int yNew)
{
	if (block)
	{
		INT x;
		...
		INT yCurMin = max(yNew - 1, 1);
		INT yCurMax = min(yNew + 1, yBoxMac);
		INT xOldMin = max(xOld - 1, 1);
		INT xOldMax = min(xOld + 1, xBoxMac);
		INT xCurMin = max(xNew - 1, 1);
		INT xCurMax = min(xNew + 1, xBoxMac);
...
VOID TrackMouse(INT xNew, INT yNew)
{
	...
	if(g_isConnected)
		SendTrackMouse(fBlock, xCur, yCur, xNew, yNew);
	{
		INT xOld = xCur;
		INT yOld = yCur;
