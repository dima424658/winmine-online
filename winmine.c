...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow)
{
	...
	hwndMain = CreateWindow(szClass, szWindowTitle,
		WS_THICKFRAME | WS_OVERLAPPED | WS_MINIMIZEBOX | WS_CAPTION | WS_SYSMENU,
...
VOID DoConnect(VOID)
{
	DialogBox(hInst, MAKEINTRESOURCE(ID_DLG_CONNECT), hwndMain, ConnectDlgProc);
}

VOID DoServer(VOID)
{
	if (CreateServer(hwndMain))
		DisplayButton(iButtonWin);
	else
		DisplayButton(iButtonLose);
}
...
INT_PTR APIENTRY ServerDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
...
INT_PTR APIENTRY ConnectDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	WCHAR out[cchNameMax];

	switch (message)
	{
	case WM_INITDIALOG:
	{
		SendMessage(GetDlgItem(hDlg, ID_TEXT_IP), EM_SETLIMITTEXT, cchNameMax, 0L);
		SetDlgItemText(hDlg, ID_TEXT_IP, L"127.0.0.1");
	}
	return (fTrue);
	case WM_CLOSE:
	{
	...
	case WM_COMMAND:
		switch (GET_WM_COMMAND_ID(wParam, lParam)) {
		case ID_BTN_CONNECT:
			GetDlgItemText(hDlg, ID_TEXT_IP, out, cchNameMax);

			int res = Connect(hwndMain, out);
			if (res)
			{
				DisplayButton(iButtonWin);
				EndDialog(hDlg, fTrue);
	...
	if (!bInitMinimized)
	{
		if (fAdjust & fResize)
		{
			MoveWindow(hwndMain, Preferences.xWindow, Preferences.yWindow,
				dxWindow + dxpBorder + 16, dyWindow + dypAdjust + 16, fTrue);
		}
